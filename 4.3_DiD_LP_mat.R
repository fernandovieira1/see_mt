########################### MODELO PORTUGUÊS 1: DiD com EF ########################### 
## Modelo Português 1: Diferenças em Diferenças com Efeitos Fixos
# Resultados em Língua Portuguesa

#### 4.1 Diferenças em Diferenças (DiD) ####
cat('\014')  # Limpar a tela do console

print("####################### RESULTADOS DIFERENÇAS EM DIFERENÇAS - LÍNGUA PORTUGUESA #######################")
...
did_model <- feols(nota_lp_q ~ see + trat_post + infra + idh_educacao | ibge_id_q + ano_q, data = base_final)
...
did_model_interacted <- feols(nota_lp_q ~ see + trat_post + infra + idh_educacao | ibge_id_q, 
                              data = base_final)
