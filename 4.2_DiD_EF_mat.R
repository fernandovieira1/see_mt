########################### MODELO MATEMÁTICA 2: DiD com EF e Ajustes ########################### 
## Modelo Matemática 2: Diferenças em Diferenças com Efeitos Fixos e Ajustes Adicionais
# Resultados em Matemática

#### 4.1 Diferenças em Diferenças (DiD) ####
cat('\014')  # Limpar a tela do console

print("####################### RESULTADOS DIFERENÇAS EM DIFERENÇAS - MATEMÁTICA (Ajustes Adicionais) #######################")
...
did_model <- feols(nota_mt_q ~ see + post + trat_post + infra + idh_educacao | ibge_id_q + ano_q, data = base_final)
...
did_model_no_time_fe <- feols(nota_mt_q ~ see + post + trat_post + infra + idh_educacao | ibge_id_q, data = base_final)
...
did_model_factor <- feols(nota_mt_q ~ see + post_factor + trat_post + infra + idh_educacao | ibge_id_q + ano_q, data = base_final)
...
did_model_adj <- feols(nota_mt_q ~ see + post_adj + trat_post + infra + idh_educacao | ibge_id_q + ano_q, data = base_final)
