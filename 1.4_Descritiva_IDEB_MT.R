...
notas_mt_ano <- ggplot(ideb_mun_mt, aes(x = ano_q, y = media_nota_mt_q, fill = dependencia_id_q)) +
  ...
  geom_text(data = medianas_mt, aes(x = ano_q, y = y_max_mt, label = paste0('Δ', round(diferenca, 2))),
            inherit.aes = FALSE, color = 'black', fontface = 'bold', vjust = -0.2) +  # Exibe a diferença entre medianas
  theme_minimal() +
  labs(title = 'Distribuição das Notas de Matemática por Ano e Dependência Administrativa', 
       x = 'Ano', y = 'Média Nota Matemática') +
  scale_fill_manual(values = c('Estadual' = 'blue', 'Municipal' = 'red')) 
...
notas_lp_ano <- ggplot(ideb_mun_mt, aes(x = ano_q, y = media_nota_lp_q, fill = dependencia_id_q)) +
  ...
  geom_text(data = medianas_lp, aes(x = ano_q, y = y_max_lp, label = paste0('Δ', round(diferenca, 2))),
            inherit.aes = FALSE, color = 'black', fontface = 'bold', vjust = -0.2) +  # Exibe a diferença entre medianas
  theme_minimal() +
  labs(title = 'Distribuição das Notas de Português por Ano e Dependência Administrativa', 
       x = 'Ano', y = 'Média Nota Português') +
  scale_fill_manual(values = c('Estadual' = 'blue', 'Municipal' = 'red'))
...
# Comparação entre escolas estaduais e municipais por ano
# Criar função para gerar gráficos filtrando por ciclo
plot_ideb <- function(data, ciclo_filtro, titulo) {
  ggplot(data %>% filter(ciclo_id_q %in% ciclo_filtro), 
    ...
    theme_minimal() +
    labs(title = titulo, x = "Ano", y = "Média IDEB") +
    scale_color_manual(values = c("Estadual" = "red", "Municipal" = "blue")) +
...
grafico_todos <- plot_ideb(ideb_mun_mt, c("AI", "AF"), "Evolução do IDEB - Todos os Ciclos")
grafico_ai <- plot_ideb(ideb_mun_mt, "AI", "Evolução do IDEB - Anos Iniciais")
grafico_af <- plot_ideb(ideb_mun_mt, "AF", "Evolução do IDEB - Anos Finais")
